...
func main() {
	...
	zClient, err := puredns.NewZClient(slog.Default(), time.Second*15, time.Second*4, 1000)
	if err != nil {
		panic(err)
	}

	lookups := zClient.Lookups(context.Background(), dns.Type(dns.TypeA), arr)
	rr := lo.Map(lookups, func(item puredns.LookupResult, index int) string {
		return item.Name
	})
