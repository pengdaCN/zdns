...
func Run2(gc GlobalConf, in <-chan string, out chan<- any) {
	...
	//if *clientsubnet_string != "" {
	//	slog.Info("使用clientsubnet_string")
	//	parts := strings.Split(*clientsubnet_string, "/")
	//	if len(parts) != 2 {
	//		log.Fatalf("Client subnet should be in CIDR format: %s", *clientsubnet_string)
	//	}
	...
	//miekgFactory := factory.(*miekg.GlobalLookupFactory)
	//slog.Info("参数", slog.Int("timeout", *timeout), slog.Int("iterationTimeout", *iterationTimeout),
	//	slog.String("class_string", *class_string), slog.String("servers_string", *servers_string),
	//	slog.String("config_file", *config_file), slog.String("localaddr_string", *localaddr_string),
	//	slog.String("localif_string", *localif_string), slog.String("clientsubnet_string", *clientsubnet_string),
	//	slog.Bool("nsid", *nsid), slog.String("type", reflect.TypeOf(factory).String()),
	//slog.String("ty", dns.TypeToString[miekgFactory.DNSType]), slog.String("class", dns.Class(miekgFactory.DNSClass).String()),
	//slog.Bool("BlacklistIsNull", miekgFactory.Blacklist == nil), slog.Bool("IterativeCacheIsNull", miekgFactory.IterativeCache == nil),
	//)
